...
/// @return A pointer to an empty double linked list on success or NULL on failure.
DoubleLinkedList* create_linked_list();

/// @brief Destroys a double linked list, freeing all the nodes and freeing the data those nodes hold.
...
/// @brief Function for in place mapping of elements in a double linked list. The first argument is the data in the data strucuture 
/// and the second is an optional data argument.
typedef void (*in_place_map_function)(void*, void*);

/// @brief In place map from tail function. Will apply a function for each element of the linked list from the tail.
...
/// @brief Iterates over all elements of the linked list, calling the function fn with the node data and extra_data as
/// arguments.
/// @param list The list itself.
