...
// I could make it such that there is a difficulty selector... maybe I do it if
// I feel like.
...
typedef struct GamePlayStateData {
    ...
    uint64_t min_acc_delta_time;  // The minimum the accumulator should be to
                                  // process the game such as moving the snake.
                                  // Changes over time.
    Apple apple;
    State* next_state;
    Snake* snake;
...
void update_game_play(State* state, uint64_t delta) {
    if (state == NULL || state->extra_data == NULL) return;

    GamePlayStateData* data = (GamePlayStateData*)(state->extra_data);

    Event* events = read_events();

    // Do nothing until the player provides the first input. I could randomize
    // it as well, but it could cause such that the player dies instantly.
    // Another alternative would be to never spawn less than n blocks to wall...
    // I prefer it like this to be honest, so I will leave as it is.
    if (data->last_button == INVALID && events == NULL) return;
    ...
    if (events != NULL) {
        Event* i = events;
        while (i != NULL) {
            // For the user input to the game, we care only about the last one,
    ...
    if (data->last_button == INVALID) return;

    size_t press_count = 0;
    for (size_t i = 0;
         i < sizeof(data->button_state) / sizeof(data->button_state[0]); ++i) {
    ...
    if (press_count > 1) {
        ...
        for (size_t i = 0;
             i < sizeof(data->button_state) / sizeof(data->button_state[0]);
             ++i) {
            data->button_state[i] = false;
        }
        return;
    }
    ...
    if (has_snake_collided(data->snake, data->side_size, data->side_size)) {
        ...
        data->min_acc_delta_time -=
            MIN_MOVE_ACC / (data->side_size * data->side_size);
        if (snake_parts_count(data->snake) ==
            data->side_size * data->side_size) {
            data->next_state = create_game_over_state(data->score);
        } else {
            data->apple =
                spawn_apple(data->snake, data->side_size, data->side_size);
        }
    }
...
State* next_state_game_play(State* state) {
    if (state == NULL || state->extra_data == NULL) return NULL;
    ...
    if (data->next_state != NULL) {
        ...
        if (data->is_next_state_pause) state->extra_data = NULL;
        return s;
    }
...
void destroy_state_game_play(State* state) {
    if (state != NULL) {
        if (state->extra_data != NULL) {
            GamePlayStateData* data = (GamePlayStateData*)(state->extra_data);
            if (data->snake != NULL) destroy_snake(data->snake);
            if (data->next_state != NULL) destroy_state(data->next_state);
            free(data);
        }
        free(state);
    }
...
State* create_game_play_state(uint32_t gameplay_area_side) {
    if (gameplay_area_side <= 1 || gameplay_area_side > INT32_MAX) return NULL;
