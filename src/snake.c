...
Snake* spawn_snake(uint32_t width, uint32_t height) {
    if (!is_valid_width_and_height(width, height)) return NULL;
...
void check_apple_overlap_function(const void* position, void* acc,
                                  void* extra_data) {
    if (position == NULL || acc == NULL || extra_data == NULL) return;

    const Point* current_part_position = (const Point*)(position);
    bool* result = (bool*)(acc);
    Apple* apple = (Apple*)(extra_data);

    if (apple->x == current_part_position->x &&
        apple->y == current_part_position->y)
        *result = true;
}
...
Apple spawn_apple(Snake* snake, uint32_t width, uint32_t height) {
    ...
    if (snake != NULL && width > 1 && height > 1 &&
        snake_parts_count(snake) < width * height) {
        bool overlaps;
        do {
            overlaps = false;
            a.x = get_rand_32() % width;
            a.y = get_rand_32() % height;
            reduce_from_tail(snake->parts_list, check_apple_overlap_function,
                             &overlaps, &a);
        } while (overlaps);
    }
    return a;
...
int has_snake_overlapped_apple(Snake* snake, Apple* apple) {
    if (snake == NULL || apple == NULL) return -1;

    Point* snake_head = (Point*)(get_tail(snake->parts_list));

    if (snake_head->x == apple->x && snake_head->y == apple->y) return 1;
...
void update_position_function(void* point, void* extra_data) {
    if (point == NULL || extra_data == NULL) return;
...
void move_snake(Snake* snake, Direction direction) {
    ...
    if (direction == INVALID_DIRECTION &&
        snake->last_direction == INVALID_DIRECTION)
        return;
    ...
    in_place_map_from_tail(snake->parts_list, &update_position_function,
                           &update_data);
}

void check_collision_function(const void* position, void* acc,
                              void* extra_data) {
    if (position == NULL || acc == NULL || extra_data == NULL) return;
    ...
    if (current_part_position->x == data->point.x &&
        current_part_position->y == data->point.y)
        *result = true;
}

int has_snake_collided(Snake* snake, uint32_t width, uint32_t height) {
    if (snake == NULL) return -1;
    if (!is_valid_width_and_height(width, height)) return -1;

    Point* snake_head_position = (Point*)(get_tail(snake->parts_list));

    if (snake_head_position == NULL) return -1;
    ...
    reduce_from_tail(snake->parts_list, &check_collision_function, &result,
                     &extra_data);
...
bool grow_snake(Snake* snake) {
    ...
    return prepend_data(snake->parts_list, &(snake->tail_last_position),
                        sizeof(Point));
}
...
void draw_fn(const void* data, void* extra_data) {
    ...
    display_draw_block(snake_part->x * dimensions->x,
                       snake_part->y * dimensions->y, dimensions->x,
                       dimensions->y);
}

void draw_game(Snake* snake, Apple* apple, uint32_t side_size) {
    // It will only support drawing integer ratios, as anything else is out of
    // scope.

    // It is fine if apple is NULL, as it simply means it is on the blinking
    // part of the cycle.
    if (snake == NULL) return;
    Point p;
    ...
    if (apple != NULL) {
        display_draw_checkerboard_block(apple->x * p.x, apple->y * p.y, p.x,
                                        p.y);
    }
}