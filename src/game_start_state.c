...
State* next_state_start(State* state) {
    ...
    if (data == NULL) return NULL;  // We return null just to point it out then
...
void destroy_state_start(State* state) {
    if (state != NULL) {
        if (state->extra_data != NULL) free(state->extra_data);
        free(state);
    }
}
...
void draw_state_start(State* state) {
    display_draw_text_justified_left(
        0, 0,
        "Press more than 1 button to play, and when "
        "playing, more than 1 button to pause.");
}
